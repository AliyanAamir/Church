	...
	useEffect(() => { 
		const fetchVideos = async () => {
			...
			try {
				...
				const res = await fetch(`https://youtube.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=25&playlistId=${process.env.REACT_APP_YOUTUBE_PLAYLIST_ID}&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`)
					.then(data => data.json())
						...
						<div className="col-12">
							<div className="section__carousel-wrap">
								<div className="section__carousel owl-carousel" id="similar">
									{
										videos.filter(x => x.snippet.title != "Deleted video").map(x => (
											<div className="card">
												<Link to={`/PreviewStream?videoId=${x.snippet.resourceId.videoId}`} className="card__cover">
												...
												<h3 className="card__title"><Link to={`/PreviewStream?videoId=${x.snippet.resourceId.videoId}`}>{x.snippet.title}</Link></h3>
												<ul className="card__list">
													<li>{new Date(x.snippet.publishedAt).toLocaleDateString("en-US", { day: "2-digit", month: "short", year: "numeric" })}</li>
												</ul>
											</div>
										))
									}
									...
									{/* <div className="card">
										...
										<h3 className="card__title"><a href="#">Live Church</a></h3>
										<ul className="card__list">
											<li>Free</li>
											<li>Action</li>
											<li>2019</li>
